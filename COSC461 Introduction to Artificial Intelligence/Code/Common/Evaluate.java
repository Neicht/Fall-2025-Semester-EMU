...
public class Evaluate
{
    ...
    private final int LIMIT = 3;                   //depth limit
    ...
    public void play()
    {
        while (!full(board))                               //computer and player take turns
        {
            board = playerMove(board);             //player makes a move
            board = computerMove(board);           //computer makes a move

        }
        System.out.println(playerWin(board) ? "Player wins" : (computerWin(board) ? "Computer Wins" : "Draw"));
    }
