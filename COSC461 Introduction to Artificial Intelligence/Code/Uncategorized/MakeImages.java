...
public class MakeImages {
    ...
    public MakeImages() {
        ...
        this.path = "/Users/nicholas/IdeaProjects/RemoteDevelopment/COSC461 Introduction to Artificial Intelligence/Code/Project2/q2/program/Data/";
    ...
    private void setupImageMenu(Project2.q2.program.TerminalInterface t, Project2.q2.program.TerminalInterface.MenuNode root) {
        ...
        t.addOption(imageMenu, "Load 'Two'", "Load the unit two image", (iface) -> {
            MakeImages.this.image.setData(ImagePreset.TWO.getDeepCopy());
            iface.out("Image set to unit '2'.");
            MakeImages.this.image.display();
        });
    ...
    private void setupBatchMenu(Project2.q2.program.TerminalInterface t, Project2.q2.program.TerminalInterface.MenuNode root) {
        ...
        t.addOption(batchMenu, "Generate Training File", "Generate 20+ images (zeros, ones, twos)", (iface) -> {
            ...
            int numTwos = iface.inInt("How many '2' images? (e.g., 10)");
            iface.out("Enter filename (e.g., training.txt):");
            String fileName = iface.inString();

            runBatchGeneration(iface, fileName, numZeros, numOnes, numTwos, "TRAIN");
        });

        t.addOption(batchMenu, "Generate Test File", "Generate 5+ images (zeros, ones, twos)", (iface) -> {
            ...
            int numTwos = iface.inInt("How many '2' images? (e.g., 2)");
            iface.out("Enter filename (e.g., test.txt):");
            String fileName = iface.inString();

            runBatchGeneration(iface, fileName, numZeros, numOnes, numTwos, "TEST");
        });
    }
    ...
    private void fileWriteTrainingHeader(String outFile, int recordCount) throws IOException {
        try (PrintWriter writer = new PrintWriter(new FileWriter(outFile, false))) {
            writer.println(recordCount + " " + 256 + " " + 3);
            writer.println();
        }
    }
    ...
    private void runBatchGeneration(Project2.q2.program.TerminalInterface iface, String fileName, int numZeros, int numOnes, int numTwos, String setType) {
        ...
        for (int i = 0; i < numTwos; i++) labels.add("two");
    ...
    private void setupProgramMenu(Project2.q2.program.TerminalInterface t, Project2.q2.program.TerminalInterface.MenuNode root) {
        ...
        t.addOption(programMenu, "Status", "Display all current settings and image status", (iface) -> {
            ...
            } else if (Arrays.deepEquals(currentData, ImagePreset.TWO.getData())) {
                iface.out("Image: Loaded 'TWO' preset.");
