...
//            return true;
    ...
    {
        String twos, threes;
        twos = Arrays.toString(new char[]{symbol, symbol});
        threes = Arrays.toString(new char[]{symbol, symbol, symbol});

        if(Arrays.toString(board.array[i]).matches(twos) || Arrays.toString(board.array[i]).matches(threes)){
            return true;
        }
    ...
    private boolean checkColumn(Board board, int i, char symbol)
    {
        ...
        twos = Arrays.toString(new char[]{symbol, symbol});
        threes = Arrays.toString(new char[]{symbol, symbol, symbol});
        char[] column = new char[board.array.length];

        for (int j = 0; j < size; j++) {
           column[j] = board.array[i][j];
        }
        if(Arrays.toString(column).matches(twos) || Arrays.toString(column).matches(threes)){
            return true;
        }
