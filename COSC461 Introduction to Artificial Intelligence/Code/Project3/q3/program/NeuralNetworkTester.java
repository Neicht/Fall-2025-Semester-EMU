...
//            processInput(t);
//        });
        t.addOption(t.getRootMenu(), "Quick Run", "Find best parameters", (iface) -> {
            try {
                tune(t);
            } catch (IOException e) {
...
//        int[] middlesToTest = {5, 10, 20};
        ...
        for (double rate : ratesToTest) {
            ...
            for (int iterations : iterationsToTest) {
                ...
                for (int middle : middlesToTest) {
                    ...
                    labels[testCount - 1] = String.format("|%.2fr|%5si|%2sm|", rate, iterations, middle);
        ...
        // make temp network to correct outputfile, reflecting the best parameters
